...
/**
   ...
   * @returns {string}
   */
export function cnAny(...args: (string | stringKeyBool | boolean | (string | boolean)[])[]): string {
    const data = args.reduce((str, curr) => {
        const x = toVal(curr);
        return str + (str ? ' ' : '') + x;
    }, '');
...
function toVal(mix: string | stringKeyBool | boolean | (string | boolean)[]): string {
    if (typeof mix === 'string') return mix;
    if (typeof mix === 'boolean') return '';
    if (Array.isArray(mix)) return mix.map(toVal).filter(Boolean).join(' ');
    return Object.keys(mix).filter((key: string) => mix[key]).join(' ');
}